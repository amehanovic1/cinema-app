...
const SummaryRow = ({ label, value, testId }) => (
    <div data-testid={testId}>
        <h3 className="text-neutral-400 text-sm md:text-base font-regular" data-testid={`${testId}-label`}>
            {label}
        </h3>
        <p
...
const PaymentDetails = ({ bookingData }) => {
    ...
    return (
        <>
            <div className="m-4 md:m-8 flex gap-8 justify-start" data-testid="payment-page-wrapper">
                <div className="flex flex-col flex-1" data-testid="payment-method-section">
                ...
                <div className="flex flex-col flex-1 max-w-md" data-testid="payment-summary-section">
                    <h1 className="text-neutral-500 font-bold text-lg md:text-xl" data-testid="payment-summary-section-title">
                        Booking Summary
                    </h1>
                    <div className="flex flex-col gap-4 bg-neutral-800 rounded-2xl shadow-lg overflow-hidden p-4">

                        <div className="flex gap-2" data-testid="payment-summary-header">
                            <img
                                src={posterUrl}
                                alt={title}
                                className="w-32 h-32 object-cover aspect-square rounded-lg shadow-md"
                                data-testid="payment-summary-movie-poster"
                            />

                            <div className="flex flex-col gap-2 text-neutral-25">
                                <h1 className="font-bold text-lg md:text-xl" data-testid="payment-summary-movie-title">
                                    {title}
                                </h1>
                                <div className="flex flex-wrap gap-3 font-normal text-sm md:text-base" data-testid="payment-summary-movie-specs">
                                    <span data-testid="payment-summary-movie-rating">{pgRating}</span>
                                    <span className="text-dark-red">|</span>
                                    <span data-testid="payment-summary-movie-language">{getLanguageName(language)}</span>
                                    <span className="text-dark-red">|</span>
                                    <span data-testid="payment-summary-movie-duration">{durationInMinutes} Min</span>
                                </div>
                            </div>
                        ...
                        <SummaryRow
                            ...
                            testId="payment-summary-date-time"
                        />

                        <SummaryRow
                            label="Cinema Details"
                            value={`${venueName} : ${street} ${streetNumber},  ${city.name} \n ${hallName}`}
                            testId="payment-summary-cinema-details"
                        />

                        <SummaryRow
                            label="Seat(s)"
                            value={formattedSeats}
                            testId="payment-summary-seats"
                        />

                        <SummaryRow
                            label="Price Details"
                            value={totalPrice.toFixed(2)}
                            testId="payment-summary-total-price"
                        />
