import axios from "axios";

const API_URL = process.env.REACT_APP_API_URL + "/movies";

export async function getCurrentlyShowingMovies(page = 0, size = 4) {
    try {
        const response = await axios.get(`${API_URL}/currently-showing?page=${page}&size=${size}`);
        return response.data;
    }
    catch (error) {
        console.log("Error fetching currently showing movies:", error);
        throw error;
    }
}

export async function getUpcomingMovies(page = 0, size = 4) {
    try {
        const response = await axios.get(`${API_URL}/upcoming?page=${page}&size=${size}`);
        return response.data;
    }
    catch (error) {
        console.log("Error fetching upcoming movies:", error);
        throw error;
    }
}